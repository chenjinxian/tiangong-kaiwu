...
export class OpenCloudIpcHandler extends IpcHandler implements OpenCloudIpcInterface {
  ...
  private async _doDownloadBriefcase(
    ...
    // The baseline file from webhook-agent is already properly created with the correct
  ...
  private _findDb(fileName: string): BriefcaseDb | undefined {
    ...
    // Try resolving relative to the service working directory if the bare name fails
    if (!db && !path.isAbsolute(fileName)) {
