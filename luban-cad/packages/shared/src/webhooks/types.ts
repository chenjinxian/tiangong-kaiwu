...
// =============================================================================
// Content payload aliases (envelope-free)
//
// webhook-agent's handler registry uses `(event, content)` pairs; these
// aliases expose the official content shapes without the envelope so both
// sides share ONE definition (T1.2 contract unification).
// =============================================================================

export type IModelCreatedContent = IModelCreatedEvent['content'];
export type IModelDeletedContent = IModelDeletedEvent['content'];
export type ChangesetPushedContent = ChangesetPushedEvent['content'];
export type NamedVersionCreatedContent = NamedVersionCreatedEvent['content'];
export type MemberAddedContent = MemberAddedEvent['content'];
export type MemberRemovedContent = MemberRemovedEvent['content'];
export type MemberRoleUpdatedContent = MemberRoleUpdatedEvent['content'];

// =============================================================================
// Local extension contracts (no official analogue — documented deviations)
// =============================================================================

/**
 * Internal HUB→webhook-agent contract: iModelCreated with the local
 * `needBaseline` flag (seeded subscriptions mark iModels whose baseline must
 * be generated by the agent). Official iTwin Platform has no such event.
 */
...
/**
 * Internal webhook-agent→modeling-server progress contract for background
 * baseline jobs (POST /api/imodels/:id/progress, X-API-Key authenticated).
 */
export interface IModelProgressPayload {
  step: string;
  progress: number;
