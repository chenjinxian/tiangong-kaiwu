...
**统计口径**: `git diff master...dev/luban-cad --stat -- ':!luban-cad' ':!common/config/rush/pnpm-lock.yaml'`（2026-09-22 实测）。汇总的 +10,963/-58 为插入/删除行；逐文件括号数字为 `--stat` 变更行数（增+删），两者关系：各文件变更行合计 11,021 = 10,963 + 58。
...
| `rush.json` | +51 | luban-cad 项目注册 + reviewCategory（`224b47a213`） |
...
每次 upstream 同步的已知坑清单见 `luban-cad/docs/UPSTREAM_SYNC.md` §「冲突处理经验（2026-09 同步 5.9→5.14 时踩过的坑）」，共 6 条：
...
将 §1.1 类（复活的 editor 工具，7,631 行）连同 §1.2 的接口扩展逐步**迁出上游 `editor/` 目录，收编至 luban-cad 独立包**，使上游 editor 包回归零修改面，把同步税从"每轮合并持续冲突"降为"独立包自行演进"。

战略论证、量化指标（T7 同步税触发条件、Phase 0 结构债对冲）与路线对比**详见 platform-docs（天工开物文档总仓，仓外）`KERNEL-STRATEGY.md` §4 成本行与 §6 触发条件**——本文件只记事实台账，一事一源，不重复战略推理。
