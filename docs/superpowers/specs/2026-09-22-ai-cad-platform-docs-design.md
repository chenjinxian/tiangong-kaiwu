...
**范围**: 跨仓库生态重分析 + 新建 platform-docs 总仓 + luban-cad 文档修正（纯文档交付，不含代码实施）
...
| 2 | 本轮交付物 | **两层都做**：新建跨仓库顶层文档 + 重写 luban-cad 过时文档 |
...
- **Phase 2**：luban-cad 文档修正（对齐代码现实 + AI+CAD 新定位）
...
每仓固定模板 → `analysis/YYYY-MM-DD-<repo>.md`：**职责 / 技术栈与规模 / 成熟度 / 接口面 / 上下游依赖 / 与 AI+CAD 的关联点 / 风险与状态**。汇总层交叉验证：客户端库 ↔ imodelhub-services 协议对齐、luban-cad ↔ 服务调用链闭合。
...
`STATUS.md` 为防脱节机制性锚点：所有能力声明必须带状态标记（§4-§2 定义的体系），与 luban-cad 文档双向引用。

产品名暂沿用 "LubanCAD"；是否更名随 AI+CAD 定位在 VISION 审阅时决定。
...
2. **G1 能力缺口门**：以 acis-solid-kernel 的 TEST_RESULTS/DS golden 完成度为基准，对照 imodel-native 实际调用面（luban-cad + itwinjs-core 所需几何能力清单），量化"重写完成度 vs 产品最小所需集"。
3. **G2 AI 需求门**：L3 生成式几何若要求可微几何/批量实例化/mesh-B-rep 混合管线，评估 imodel-native 是否本就无法满足——若是，自研/OCCT 成为必由之路。

**交付形态**：对比矩阵 + 各路线阶段图（并行期/切换点/回退条件）+ 明确推荐 + 换路线触发条件清单。证据源：acis-solid-kernel 测试报告、imodel-native 能力文档、FreeCAD GCS/Sketcher 分析、luban-cad 几何调用面实测。

### §4 Phase 2：luban-cad 文档修正
...
**执行顺序**：Phase 1a 并行勘察 → Phase 1b 汇总写 platform-docs → Phase 2 修正 luban-cad → 每阶段 git 提交。
...
| 10 | 仓库卫生问题 | `luban-cad/.github/workflows` 死配置；`__blobstorage__`/playwright-report/`web-agent.log` 入库 | 记入技术债 |
