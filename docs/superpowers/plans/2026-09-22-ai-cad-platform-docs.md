...
**Goal:** 完成 11 个仓库的生态重分析，产出 platform-docs 顶层文档体系（AI+CAD 愿景/架构/内核战略/路线图/能力矩阵），并把 luban-cad 的 15 份文档修正到与代码一致 + 新定位。

**Architecture:** 两阶段纯文档交付：Phase 1（Task 1–11）新建独立 git 仓 `~/Documents/GitHub/platform-docs`，先并行勘察 3 批仓库再汇总成 8 份文档；Phase 2（Task 12–15）在 itwinjs-core 仓（`luban-cad/` 下）按验收清单修正文档。所有能力声明必须带状态标记，每个事实声明需可溯源（file:line 或分析笔记）。

**Tech Stack:** Markdown 文档 + git；勘察使用 Bash（ls/find/wc/grep/git log）+ Read 工具。

**Spec:** `luban-cad/docs/superpowers/specs/2026-09-22-ai-cad-platform-docs-design.md`（本计划的唯一需求来源）

## Global Constraints

- **状态标记体系**（每个能力声明必须带其一）：`✅ 真实可用` / `🟠 部分实现` / `⚪ stub 或 mock` / `❌ 禁用或失效`
- **受众**：开发团队 + AI 助手；工程视角、状态诚实化；platform-docs 维护 `AGENTS.md` 供 AI 助手消费
- **一事一源**：跨仓愿景/架构只写在 platform-docs；luban-cad 文档引用不复制
- **产品名**暂沿用 `LubanCAD`（更名留待 VISION 审阅决策，不在本计划内执行）
...
- **仓库边界**：Task 1–11 提交到新仓 `~/Documents/GitHub/platform-docs`；Task 12–15 提交到 `/Users/xunzhang/Documents/GitHub/itwinjs-core`（分支 `dev/luban-cad`）
...
| 事实 | 值 / 证据 |
|---|---|
| luban-cad 源码规模 | ~37,584 行 TS/TSX（features 15,045 / packages 7,731 / web/src 6,883 / backend 4,061 / web-agent 2,743 / modules 1,121），58 个测试文件，13 个 feature 模块 |
| 服务拓扑 | web:3000 / imodelhub-services:4000 / backend:4001 / web-agent:4002 / PostgreSQL:5432 / Azurite:10000-10002 |
| fork 对上游的修改面 | 20 文件 +10,963 行（`git diff master...dev/luban-cad -- ':!luban-cad' ':!common/config/rush/pnpm-lock.yaml'`）；最大块：`editor/frontend/src/{SketchTools +2163, SolidModelingTools +1807, SolidPrimitiveTools +1784, ModifyCurveTools +969, ElementGeometryTool +908, TransformElementsTool +511}`、`editor/common/src/EditorBuiltInIpc.ts +583` |
...
| 仓库卫生 | `luban-cad/.github/workflows/`（cd/ci/pr）是死配置（GitHub 只认仓库根 `.github/`）；`apps/web/{playwright-report,test-results,__blobstorage__}` 入库；itwinjs-core 根有游离 `web-agent.log` |
...
LubanCAD —— 基于云服务的 **AI+CAD** 平台 —— 的跨仓库架构与愿景文档。
...
## luban-cad 开发准则（继承自其 CLAUDE.md）

- 复用优先：工具类功能必须继承 `ElementSetTool`/`PrimitiveTool`/`CopyElementsTool` 等基类
- itwinjs-core 源码修改必须最小化并完整记录于 `luban-cad/docs/ITWINJS_CORE_MODIFICATIONS.md`
...
必填要点：`web-service-backend` 与 `web-agent-backend` 各自职责（与 luban-cad 的 web-agent:4002 是否同源/重复——如实记录）；rush 包数量。
...
必填要点：支持的后端（Azure/S3/MinIO/本地？）；luban-cad 当前直连 Azurite 的事实与该抽象的关系（可替换点）。
...
必填要点：iModel 文件 IO / ECSQL / 变更追踪 / 几何处理四大能力的模块归属；**几何能力清单**（圆角/倒角/抽壳/扫掠/布尔/放置变换——luban-cad 实际用到的最小集，对照 `luban-cad/apps/web/features/modeling/` 的工具列表）；许可与商业依赖性质（G1 门输入）。
...
### Task 5: itwinjs-core / luban-cad 现状笔记（固化已知事实）

**Files:**
- Create: `/Users/xunzhang/Documents/GitHub/platform-docs/analysis/2026-09-22-itwinjs-core-luban-cad.md`
...
find luban-cad -type f \( -name '*.ts' -o -name '*.tsx' \) -not -path '*/dist/*' -not -path '*/lib/*' -not -name '*.test.*' -not -name '*.bak' | xargs wc -l | tail -1
git diff master...dev/luban-cad --stat -- ':!luban-cad' ':!common/config/rush/pnpm-lock.yaml' | tail -1
find luban-cad -type f \( -name '*.test.ts' -o -name '*.test.tsx' \) -not -path '*/dist/*' | wc -l
grep -c 'IModelApp.tools.register' luban-cad/apps/web/features/editor/registerTools.ts
```
...
```bash
cd /Users/xunzhang/Documents/GitHub/platform-docs
git add analysis/ && git commit -m "docs(analysis): freeze itwinjs-core/luban-cad current-state findings and drift list
...
11 行：itwinjs-core(含 luban-cad)、imodel-native、imodelhub-services、platform-backend、
object-storage、imodels-clients、itwins-client、auth-clients、access-control-client、
acis-solid-kernel、FreeCAD(参考资产)

## 2. 依赖关系图
（mermaid graph TD：luban-cad → backend/web-agent → imodelhub-services / Azurite / PostgreSQL；
...
## 4. 调用链验证
（luban-cad 的 REST/RPC/Webhook/IPC 四条链路与服务端点的闭合性，数据来自 Task 5 笔记）
...
- Produces: 三层 AI 形态的权威定义——Task 8/10 与 luban-cad README（Task 12）引用
...
- 缺口表：能力 | acis-solid-kernel | imodel-native | luban-cad 需求方
...
- Produces: 分期计划与还债清单——luban-cad ROADMAP（Task 14）向上对齐它
...
依赖：约束服务（GCS 移植或商业 DCM，按 KERNEL-STRATEGY）、特征树重生成引擎（luban-cad 既有 CadFeature 骨架）
...
## 与旧版 ROADMAP 的关系
旧版（luban-cad/docs/ROADMAP.md，2026-04）以"对标 Onshape"为目标；本版以 AI+CAD 三层为目标，
...
### Task 12: luban-cad README + CLAUDE.md 诚实化 + 新定位

**Files:**
- Modify: `/Users/xunzhang/Documents/GitHub/itwinjs-core/luban-cad/README.md`
- Modify: `/Users/xunzhang/Documents/GitHub/itwinjs-core/luban-cad/CLAUDE.md`
...
偏差清单 #1、#7 两项在 Task 5 笔记中标 `已消项`（在 platform-docs 仓改 `analysis/2026-09-22-itwinjs-core-luban-cad.md` 的清单状态列，单独提交）。

- [ ] **Step 4: 提交（两仓各一次）**

```bash
cd /Users/xunzhang/Documents/GitHub/itwinjs-core
git add luban-cad/README.md luban-cad/CLAUDE.md
...
**Files:**
- Rewrite: `/Users/xunzhang/Documents/GitHub/itwinjs-core/luban-cad/docs/ITWINJS_CORE_MODIFICATIONS.md`
...
```bash
cd /Users/xunzhang/Documents/GitHub/itwinjs-core
git diff master...dev/luban-cad --stat -- ':!luban-cad' ':!common/config/rush/pnpm-lock.yaml'
```
...
**汇总**: 20 个文件，+10,963 行 / -58 行（不含 luban-cad/ 与 lockfile）
...
## 3. 战略建议
将 1.1 类迁出 editor/ 至 luban-cad 独立包（详见 platform-docs/KERNEL-STRATEGY.md 同步税段）
## 4. 自动生成文件
...
### Task 14: luban-cad ROADMAP + DEEP_ANALYSIS + Architecture 更新（偏差 #5 消项）

**Files:**
- Rewrite: `/Users/xunzhang/Documents/GitHub/itwinjs-core/luban-cad/docs/ROADMAP.md`
- Modify: `/Users/xunzhang/Documents/GitHub/itwinjs-core/luban-cad/docs/DEEP_ANALYSIS.md`
- Modify: `/Users/xunzhang/Documents/GitHub/itwinjs-core/luban-cad/docs/Architecture.md`
...
结构：开头加一段 `本文档是平台 ROADMAP（platform-docs/ROADMAP.md）在 luban-cad 应用侧的落点`；
...
**Files:**
- Modify（校对）: `luban-cad/docs/{UPSTREAM_SYNC.md,CONSTRAINT-SOLVER-COMPARISON.md,SKETCH-CONSTRAINT-WORKFLOW.md,IMODEL_AUTO_RECOVERY_DESIGN.md,baseline-reliability-design.md,WEBHOOK_CONFIG.md,Deployment-Guide.md}`
- Modify（加存档横幅）: `luban-cad/docs/{E2E_TEST_REPORT.md,docs/README.md 中过时段落}` —— 以逐文件核对结论为准：判定标准 = 文中数据/端口/功能状态与 2026-09 实测不符则加横幅
- Modify: `/Users/xunzhang/Documents/GitHub/platform-docs/analysis/2026-09-22-itwinjs-core-luban-cad.md`（偏差清单终态）
...
```bash
cd /Users/xunzhang/Documents/GitHub/itwinjs-core/luban-cad
