 ...
 *
 * Open Cloud CAD Backend Service
...
async function initializeBackend() {
  logger.info('Initializing Open Cloud CAD Backend...');
...
function setupServer(rpcConfig: BentleyCloudRpcConfiguration): http.Server {
  ...
  (app as any).ws('/ws', (ws: WebSocket, _req: Request) => {
    ...
    ws.send(JSON.stringify({
      type: 'connected',
      message: 'Connected to Open Cloud CAD Backend',
  ...
  // ============================================
  // Webhook Event Receiver (from web-agent)
  // ============================================
  ...
  app.post('/api/imodels/:id/retry', express.json(), async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.status(502).json({ error: 'Failed to communicate with web-agent' });
    }
  });
  ...
  app.post('/api/imodels/:id/progress', (req: Request, res: Response) => {
    // Validate API key from web-agent
  ...
  // ============================================
  server.listen(PORT, () => {
    logger.info(`Open Cloud CAD Backend running on http://localhost:${PORT}`);
