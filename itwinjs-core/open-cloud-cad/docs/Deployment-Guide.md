...
Open Cloud CAD 使用 **imodelhub-services 模式**，所有服务在本地或私有环境运行，**不依赖 Bentley iTwin Platform 云服务**。
...
| Backend | 4001 | Open Cloud CAD 后端 | 是 |
| Web | 3000 | 前端应用 | 是 |
...
services:
  azurite:
    ...
    container_name: open-cloud-cad-azurite
    ports:
      - "10000:10000"
      - "10001:10001"
      - "10002:10002"
    ...
    networks:
      - open-cloud-cad-network

  backend:
    build:
      context: .
      dockerfile: apps/backend/Dockerfile
    container_name: open-cloud-cad-backend
    ports:
      - "4001:4001"
    environment:
      - NODE_ENV=production
  ...
  web-agent:
    ...
    container_name: open-cloud-cad-web-agent
    ports:
      - "4002:4002"
    environment:
      - NODE_ENV=production
  ...
  web:
    ...
    container_name: open-cloud-cad-web
    ports:
      - "3000:3000"
    environment:
      - VITE_API_URL=http://localhost:4001
...
# 推送镜像到仓库
docker tag open-cloud-cad-web:latest your-registry/open-cloud-cad-web:v1.0
docker push your-registry/open-cloud-cad-web:v1.0
```
...
```bash
# Backend 日志
docker logs -f open-cloud-cad-backend

# Web-Agent 日志
docker logs -f open-cloud-cad-web-agent
...
```bash
# PostgreSQL 备份
docker exec -t open-cloud-cad-postgres pg_dump -U imodelhub imodelhub > backup.sql
...
```bash
# PostgreSQL 恢复
docker exec -i open-cloud-cad-postgres psql -U imodelhub imodelhub < backup.sql
...
1. **Backend 启动失败**
   ...
   - 检查 Azurite 是否运行
   - 检查端口是否被占用
   - 查看日志: `docker logs open-cloud-cad-backend`
...
# 删除数据卷
docker volume rm open-cloud-cad_azurite-data
