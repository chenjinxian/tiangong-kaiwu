...
export class V2CheckpointManager {
  ...
  private static toCloudContainerProps(from: V2CheckpointAccessProps): CloudSqlite.ContainerAccessProps {
    // Support custom blob base URI for Azurite/local development via environment variable
    const baseUri = process.env.IMJS_AZURE_BLOB_BASE_URI ?? `https://${from.accountName}.blob.core.windows.net`;
    return { ...from, baseUri, accessToken: from.sasToken, storageType: "azure" };
  }
