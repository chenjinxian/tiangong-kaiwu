...
export abstract class SolidPrimitiveTool extends CreateElementWithDynamicsTool {
  ...
  protected override async doCreateElement(props: GeometricElementProps): Promise<void> {
    try {
      ...
      // NOTE: Finish command to ensure onGeometryChanged event is properly triggered.
      // Without this, the EditCommand remains active and may prevent event propagation.
      await EditTools.finishCommand();
      this._startedCmd = undefined; // Reset started command so next element creates a new command
    } catch (err) {
